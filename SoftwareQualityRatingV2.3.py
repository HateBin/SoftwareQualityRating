...
def create_plot(func):
    ...
    def wrapper(*args, **kwargs) -> dict:
        ...
        try:
            ...
            # 返回图表路径和元数据
            return {
                'plotPath': upload_file(buf.getvalue()),  # 上传到文件至tapd服务器返回一个图片的在线路径
...
class SoftwareQualityRating:
    def __init__(self):
        ...
        self.earliestTestTaskDate: str = ''  # 最早测试任务日期
    ...
    def add_test_report(self) -> None:
        ...
        # 测试报告中的评分总分
        score = '<span style="color:#ff3b30;">' + str(sum(self.score.values())) + "分</span>" if sum(self.score.values()) < 60 else str(sum(self.score.values())) + "分"
        ...
        test_conclusion: dict[str, str] = {
            ...
            # 组合各维度评分
            '软件提测质量评分': f'{score}（配合积极性/文档完整性{self.score["positiveIntegrityScore"]}分 + 冒烟测试{self.score["smokeTestingScore"]}分 + BUG数{self.score["bugCountScore"]}分 + BUG修复速度{self.score["bugRepairScore"]}分 + BUG重启率{self.score["bugReopenScore"]}分）',
            '测试时间': '',  # 预留字段，需后续补充
